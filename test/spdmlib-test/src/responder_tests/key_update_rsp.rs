...
// Responder receive correct KEY_UPDATE request and send correct KEY_UPDATE_ACK response, expect keys updated.
#[test]
fn test_case2_handle_spdm_key_update() {
    let future = async {
        ...
        let origin_key = context.common.session[0].export_keys();
        ...
        // send SpdmUpdateAllKeys requset tag = 1
        let key_exchange: &mut [u8; 1024] = &mut [0u8; 1024];
        let mut writer = Writer::init(key_exchange);
        let value = SpdmKeyUpdateRequestPayload {
            key_update_operation: SpdmKeyUpdateOperation::SpdmUpdateAllKeys,
        ...
        // send SpdmVerifyNewKey requset tag = 2
        let key_exchange: &mut [u8; 1024] = &mut [0u8; 1024];
        let mut writer = Writer::init(key_exchange);
        let value: SpdmKeyUpdateRequestPayload = SpdmKeyUpdateRequestPayload {
        ...
        let mut writer = Writer::init(&mut response_buffer);
        let (status, send_buffer) = context.handle_spdm_key_update(session_id, bytes, &mut writer);
        let new_key = context.common.session[0].export_keys();

        // receiving a correct UPDATE_KEY_ACK message for updating all keys.
