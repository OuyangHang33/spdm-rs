...
async fn handle_message(
    ...
    loop {
        ...
        let (used, secured_message) = if let Ok((u,s)) = receive_message(&mut context, raw_packet).await
        {
            (u,s)
        } else {
        ...
        if secured_message {
            ...
            let spdm_session = if let Some (s) = context
                .common
            ...
            match spdm_session.get_session_state() {
                SpdmSessionState::SpdmSessionEstablished => {
                    ...
                    let message_header = spdmlib::message::SpdmMessageHeader::read(&mut reader);
