...
use crate::common::{session::SpdmSessionState, SpdmDeviceIo};
...
use spdmlib::message::{
    measurement, SpdmErrorCode, SpdmKeyUpdateOperation, SpdmKeyUpdateRequestPayload,
...
async fn handle_message(
    ...
    loop {
        ...
        match receive_message(&mut context, raw_packet).await {
            Ok((used, secured_message)) => {
                if secured_message {
                    let mut read = Reader::init(&raw_packet[0..used]);
                    let session_id = u32::read(&mut read).ok_or(used).unwrap();
                    let spdm_session = context
                        .common
                    ...
                    match spdm_session.get_session_state() {
                        SpdmSessionState::SpdmSessionEstablished => {
                            let mut spdm_buffer = [0u8; config::MAX_SPDM_MSG_SIZE];
                            let decap_result = {
                                let mut transport_encap = context.common.transport_encap.lock();
                            ...
                            let bytes = &spdm_buffer[0..decode_size];
                            let mut reader = Reader::init(bytes);
                            let message_header =
                                spdmlib::message::SpdmMessageHeader::read(&mut reader).unwrap();

                            if message_header.request_response_code
                                == spdmlib::message::SpdmRequestResponseCode::SpdmRequestKeyUpdate
                            {
                                // Handle Update Key Request.
                                let (decode_size, is_app_message) = decap_result.unwrap();
                                let mut reader = Reader::init(&spdm_buffer[0..decode_size]);

                                let bytes = &spdm_buffer[0..decode_size]; // modify here to get incorrect request
                                let (_status, send_buffer) =
                                    context.handle_spdm_key_update(session_id, bytes, &mut writer);

                                let send_buffer = if let Some(s) = send_buffer {
                                    s
                                } else {
                                    panic!("Invalid message!");
                                };
                                // use wrong key to get undecryptable garbage msg
                                let _ = send_message(
                                    &mut context,
                                    Some(session_id),
                                    send_buffer,
                                ...
                                match res {
                                    Ok(spdm_result) => match spdm_result {
                                        Ok(_) => continue,
                                        Err(status) => {
                                            panic!("process_message failed with {:?}", status)
                                        }
                                    },
                                    Err(used) => {
                    ...
                    let (status, send_buffer) =
                        context.dispatch_message(&raw_packet[0..used], &mut writer);
                    let res = if let Some(send_buffer) = send_buffer {
                        if let Err(err) = context.send_message(None, send_buffer, false).await {
                            Ok(Err(err))
                        } else {
                            Ok(status)
                        }
...
async fn receive_message(
    context: &mut responder::ResponderContext,
    receive_buffer: &mut [u8],
) -> Result<(usize, bool), usize> {
    info!("receive_message!\n");

    let timeout: usize = ST1;
    ...
    let used = if let Some(session_id) = session_id {
        context
            ...
            .encode_secured_message(
                ...
                corrupt_send_buffer, // use incorrect key to get undecryptable garbage msg
                false,
            )
            .await?
    } else {
