...
use spdmlib::message::{
    SpdmErrorCode, SpdmKeyUpdateOperation, SpdmKeyUpdateRequestPayload,
...
async fn handle_message(
    ...
    loop {
        ...
        if count == 14 {
            ...
            //         .encryption_key
            //         .data[..],
            //     &context.common.session[0].export_keys().1.sequence_number,
            // );
            ...
            //         .encryption_key
            //         .data[..],
            //     &context.common.session[0].export_keys().0.sequence_number,
            // );

            info!("receive_message!\n");
            let (used, _secured_message) = if let Ok((u,s)) = receive_message(&mut context, raw_packet).await
            {
                (u,s)
            } else {
                panic!("Invalid secured_message result!");
            };
            ...
            let decode_size = if let Ok(d) = spdm_session.decode_spdm_secured_message(
                &raw_packet[..used],
                &mut app_buffer,
                true,
            ){
            ...
            let (decode_size, is_app_message) = if let Ok((d, s)) = decap_result {
                (d, s)
            } else {
                panic!("Invalid decode result!");
            };
            ...
            let _ = send_message(&mut context, Some(session_id), send_buffer, false).await.is_ok();
